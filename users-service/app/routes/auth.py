...
def login_form_for_token(form_data: Annotated[OAuth2PasswordRequestForm, Depends()], db: Session = Depends(get_session)):
    return authenticate_user_and_get_token(form_data.username.lower(), form_data.password, db)


@router.post('/token', response_model=schemas.Token)
def login_for_token(payload: schemas.LoginPayload, db: Session = Depends(get_session)):
